...
AXL is not an API, not a framework, not a prompt template. It is a **grammar**, a 75-line BNF kernel that any LLM learns on first read. No training. No fine-tuning. No SDK.
...
<CardGroup cols={2}>
  ...
  <Card title="The Rosetta" icon="scroll" href="/rosetta/overview">
    The 75-line self-bootstrapping kernel
  </Card>
...
| Metric | Value |
| --- | --- |
| Rosetta length | 75 lines (v3 kernel) |
...
<Steps>
  <Step title="Read the Rosetta">
    Any LLM reads the 75-line kernel once. Fluent immediately.
  </Step>
