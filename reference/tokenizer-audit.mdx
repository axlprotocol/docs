...
AXL v2.1 validated its alphabet against cl100k_base (OpenAI). The audit was extended across five tokenizer families using public documentation, NFKC normalization testing, and inference from model cards. All findings remain valid in v3.
...
- `µ` (micro sign, U+00B5) normalizes to `μ` (Greek mu, U+03BC), which **breaks identity**
- `π ← → ↑ ↓` survive NFKC (safe at normalization level, but may still multi-tokenize)
- All ASCII symbols survive NFKC unchanged

## Conclusion

The universal-safe core is ASCII punctuation plus typed prefixes. Tokenizer neutrality is framed as **orthographic invariance**, not identical token counts. v3 ASCII mode guarantees parser correctness everywhere. Compression efficiency is profile-relative.
